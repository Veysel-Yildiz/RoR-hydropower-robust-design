function [AR,  AC,  WEP_1] = calc_hpp2 ( x , HP , O , nt, On )
...
if HP.hd < 0.5*HP.hg
    AR = -29999990*abs(HP.hd) ;
    AC = 29999990*abs(HP.hd);
    WEP_1 = -29999990*abs(HP.hd); return
end
...
parfor t = 1 : HP.maxT
    ...
    elseif q >=  Od1 + kmin * Od2 %&& q <= Od1 + Od2
        % large francis/kaplan/cross turbine efficiency
        ...
        % [kW] power produced, monthly
        P(t) =  ( Od1 *n1 + (q-Od1)*n2) * hnet * 9.81 * ng  ;
    end
end
...
WEP_1  = prctile(Y_p,1)*24/10^6; % not percentage
% WEP_1  = prctile(Y_p,1)*24/10^4/AAE; % percentage

AR = AAE* HP.ep*0.98; % AnualRevenue in M dollars 1% will not be sold
