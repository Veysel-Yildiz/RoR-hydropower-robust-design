...
qt = max(min(Q),mean(Q)/2);
dmin = sqrt(qt/(8*3.14)*4);

% Define parameter ranges, if 'latin'
Par_info.min = [dmin qt 0.5 ti 0.501];  % If 'latin', min values
Par_info.max = [3.00 30 10.0 tj 3.499];   % If 'latin', max values
