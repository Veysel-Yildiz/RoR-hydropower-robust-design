...
O = load('karacay_observed.txt');
O_60_50 = load('karacay_50x60_years.txt');
...
X = load('X200_karacay.txt');
X(201,:) = [0.93, 1.26, 1.26, 3, 2];%identical 10% discount of em cost
X(202,:) = [0.93, 1.26, 1.26, 3, 2];%identical

%% project variables

HP.hg = 134; % initial storage elevation (m), gross head
HP.ht = 0;  % tail water elevation, depth of outflow to stream (m)
HP.L = 1600; % the length of penstock (m)
...
HP.fxc  =  1; % the expropriation and other costs including transmission line
% MFD = 0.001; % the minimum environmental flow (m3/s)
...
for  j = 1: Ns
    ...
    for k = 1:nf
        ...
        elseif conf(k)==2
            ...
            [AR(j,k),  AC(j,k), WEP1(j,k), BC(j,k), power(j,k), AAE(j,k), T_cost(j,k), NPV(j,k), PayB(j,k), Y_ae(j*2450-2449:j*2450,k), PB10(per*j-(per-1):per*j,k),  PB15(per*j-(per-1):per*j,k), PB20(per*j-(per-1):per*j,k), nNPV(per*j-(per-1):per*j,k)] = calc_hpp3 ( X(k,:) , HP , Onew , type(k), 3 );
        end
...
save ('outputKARACAY','AR', 'AC', 'WEP1','BC','power', 'AAE', 'T_cost','NPV', 'PayB','Y_ae','multiplier','FDCpars','kpn','PB10','PB15','PB20','nNPV','-v7.3')
