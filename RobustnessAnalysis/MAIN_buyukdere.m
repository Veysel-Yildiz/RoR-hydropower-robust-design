...
O = load('buyuk_observed.txt');
O_60_50 = load('buyuk_50x60_years.txt');
...
X = load('X200_buyuk.txt');
X(201,:) = [1.07, 1.37, 1.37, 3, 2];%identical 10% discount
X(202,:) = [1.07, 1.37, 1.37, 3, 2];%identical 


%% project variables

HP.hg = 394; % gross head
HP.ht = 0;
HP.L = 7860; % the length of penstock (m)
...
HP.maxT = length(kpn);           % the size of time stepsclc; clear;
...
save ('outputBUYUK','AR', 'AC', 'WEP1','BC','power', 'AAE', 'T_cost','NPV', 'PayB','Y_ae','multiplier','FDCpars','kpn','PB10','PB15','PB20','nNPV','-v7.3')
